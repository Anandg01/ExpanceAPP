...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(!token){
    ...
    axios.get(`http://3.86.103.50:2000/allExpance?page=1`,{headers:{'Authorizan':token}})
    .then(res=>{
...
const validate=axios.post(`http://3.86.103.50:2000/addExpance `,expanceDetais,{headers:{'Authorizan':token}})
  .then(res=>{
...
function deleteClick(id){
    axios.delete(`http://3.86.103.50:2000/deleteExp/${id}`,{headers:{'Authorizan':token}})
    .then(data=>{
...
function leaderBoard(){
  axios.get('http://3.86.103.50:2000/leaderbord').then(res=>{
...
document.getElementById('rozerpay').addEventListener('click', async (e) => {
  ...
  const response = await axios.get(`http://3.86.103.50:2000/premium`, {
  ...
  const options = {
    ...
    handler:async function(response){
      axios.post(`http://3.86.103.50:2000/updatetransastion`,
 ...
 paymentButton.open();
  ...
  await  axios.post(`http://3.86.103.50:2000/updatetransastion`,
...
function download(){
  axios.get('http://3.86.103.50:2000/download', { headers: {"Authorizan" : token} })
  .then((response) => {
...
function downloadAll(){
  axios.get('http://3.86.103.50:2000/allFileurl', { headers: {"Authorizan" : token} })
  .then(fileurl=>{
...
function getProducts(page){
  axios.get(`http://3.86.103.50:2000/allExpance?page=${page}&perPage=${perPage}`,{headers:{'Authorizan':token}})
  .then(res=>{
    const parant=document.getElementById('addTable');
